...
// StorageMockWrapper is a mock storage wrapper for testing.
type StorageMockWrapper struct {
	Wrapped Storage

	RegisterMock             func(ctx context.Context, job *Job) error
	RegisterJobTxMock        func(ctx context.Context, tx QueryExecutor, job *Job) error
	...
	ClearDoneJobsMock func(ctx context.Context, processId string, activeThreshold time.Time, limit int) (int64, error)
	FilterJobsMock    func(ctx context.Context, input FilterJob, page int, itemsPerPage int) ([]*Job, int, error)
}
...
func (s *StorageMockWrapper) RegisterJobTx(ctx context.Context, tx QueryExecutor, job *Job) error {
	if s.RegisterJobTxMock != nil {
		return s.RegisterJobTxMock(ctx, tx, job)
	}
	return s.Wrapped.RegisterJobTx(ctx, tx, job)
}
...
func (s *StorageMockWrapper) ClearDoneJobs(ctx context.Context, processId string, activeThreshold time.Time, limit int) (int64, error) {
	if s.ClearDoneJobsMock != nil {
		return s.ClearDoneJobsMock(ctx, processId, activeThreshold, limit)
	}
	return s.Wrapped.ClearDoneJobs(ctx, processId, activeThreshold, limit)
}

func (s *StorageMockWrapper) FilterJobs(ctx context.Context, input FilterJob, page int, itemsPerPage int) ([]*Job, int, error) {
	if s.FilterJobsMock != nil {
		return s.FilterJobsMock(ctx, input, page, itemsPerPage)
	}
	return s.Wrapped.FilterJobs(ctx, input, page, itemsPerPage)
}
