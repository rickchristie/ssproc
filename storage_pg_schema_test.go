...
func TestSchemaVerifier(t *testing.T) {
	...
	runRows := func(t *testing.T, rows []*testRow) {
		for _, r := range rows {
			t.Run(r.name, func(t *testing.T) {
				...
				_, err = conn.Exec(ctx, r.createTableInput)
				assert.Nil(t, err)

				storage, err := NewPgStorage(PgStorageConfig{
					ConnStrSelector: testutil.Randomized([]string{db.ConnString}),
					Schema:          schema,
					Table:           table,
				})
	...
	t.Run("validates and reject if table schema is not as expected", func(t *testing.T) {
		runRows(t, []*testRow{
			{
				...
				CREATE INDEX schema_verifier_cleanup_idx on public.schema_verifier (process_id, status, end_ts);`,
				expected: &expected{
					valid: true,
				},
			},
			...
			},
			{
				...
				CREATE INDEX schema_verifier_cleanup_idx on public.schema_verifier (process_id, status, end_ts);`,
				expected: &expected{
					errorMsg: "public.schema_verifier: multiple indexes found, expected three indexes",
				},
			},
			{
			...
			},
			{
				...
				createTableInput: `CREATE TABLE public.schema_verifier (
					...
					job_data text NOT NULL DEFAULT '',
					process_id text NOT NULL,
					goroutine_id varchar(512) NOT NULL,
					goroutine_heart_beat_ts timestamp with time zone,
					goroutine_lease_expire_ts timestamp with time zone,
					status varchar(64) NOT NULL DEFAULT 'ready',
					next_subprocess int NOT NULL DEFAULT 0,
			...
			// Failure cases, incorrect max length.
			{
				...
				CREATE INDEX schema_verifier_cleanup_idx on public.schema_verifier (process_id, status, end_ts);`,
				expected: &expected{
					errorMsg: "public:schema_verifier expected column goroutine_id char max length to be 512, found 127",
				},
			},
			{
				name: "incorrect max length 129",
				createTableInput: `CREATE TABLE public.schema_verifier (
					job_id text NOT NULL PRIMARY KEY,
					job_data text NOT NULL DEFAULT '',
					process_id text NOT NULL,
				...
				CREATE INDEX schema_verifier_cleanup_idx on public.schema_verifier (process_id, status, end_ts);`,
				expected: &expected{
					errorMsg: "public:schema_verifier expected column goroutine_id char max length to be 512, found 129",
				},
			},
	...
	t.Run("does not return error when table is valid", func(t *testing.T) {
		state := StateCreator()
		state.Setup(t)
		defer state.TearDown(t)

		state.InitValidDb(t, schema, table)
		storage, err := NewPgStorage(PgStorageConfig{
			ConnStrSelector: testutil.Randomized([]string{state.Db.ConnString}),
			Schema:          schema,
			Table:           table,
		})
		assert.Nil(t, err)
