...
func TestSchemaVerifier_DoesNotReturnError_WhenValid(t *testing.T) {
	s := StateCreator()
	s.Setup(t)
	defer s.TearDown(t)
	...
	storage, err := NewPgStorage(
		s.Db.ConnString,
		schema, table,
		plugs.DefaultLogger("Test_DoesNotReturnError_WhenTableIsValid"),
		util.NewGlobalTime(time.Local),
	)
	assert.Nil(t, err)
	assert.NotNil(t, storage)
}

func TestSchemaVerifier_ReturnError_WhenInvalid(t *testing.T) {
	s := StateCreator()
	s.Setup(t)
	defer s.TearDown(t)
	...
	runRows := func(t *testing.T, rows []*testRow) {
		for _, r := range rows {
			t.Run(r.name, func(t *testing.T) {
				...
				logger := plugs.DefaultLogger(r.name)
				utilTime := util.NewGlobalTime(time.Local)
				storage, err := NewPgStorage(s.Db.ConnString, schema, table, logger, utilTime)
